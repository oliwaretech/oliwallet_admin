...
class RegisterApproveLoanPeriodContent extends ConsumerStatefulWidget {
  ...
  const RegisterApproveLoanPeriodContent({super.key,
    required this.countryLoan,
    required this.onAdd,
  });

  @override
  ConsumerState<RegisterApproveLoanPeriodContent> createState() => _RegisterApproveLoanPeriodContentState();
}

class _RegisterApproveLoanPeriodContentState extends ConsumerState<RegisterApproveLoanPeriodContent> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Text(appLocalizations.selectThePeriodApplicableToThisRange, style: OlwTextStyles.textCardTitle),
        Wrap(
            runSpacing: 8,
            spacing: 8,
            children: widget.countryLoan.approvalConfig.paymentFrequencies.map((paymentFrequency) {
              return OlwChip(
                  isSelected: selectedPaymentFrequency == paymentFrequency,
                  onTap: (){
                    setState(() {
                      selectedPaymentFrequency = paymentFrequency;
                      minInstallmentsController.text = paymentFrequency.minInstallments.toString();
                      maxInstallmentsController.text = paymentFrequency.maxInstallments.toString();
                      gracePeriodController.text = paymentFrequency.gracePeriods.first.toString();
                    });
                  },
                  label: paymentFrequency.name.getLocalized(appLocalizations.localeName));
            }).toList(
            )),
        Row(
          spacing: 16,
          children: [
            Expanded(
              child: Column(
                ...
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(appLocalizations.minInstallments, style: OlwTextStyles.textCardTitle),
                  OlwTextField(
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(appLocalizations.maxInstallments, style: OlwTextStyles.textCardTitle),
                  OlwTextField(
        ...
        Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(appLocalizations.gracePeriod, style: OlwTextStyles.textCardTitle),
            OlwTextField(
        ...
        OlwPrimaryButton(
            onPressed: (){
              ...
              if(minInstallmentsController.text.isEmpty || maxInstallmentsController.text.isEmpty || gracePeriodController.text.isEmpty){
                OlwSnackBarNotification.showInfo(
                    title: appLocalizations.somethingIsMissing,
              ...
              for (var i = 0; i < int.parse(gracePeriodController.text)+1; i++) {
                gracePeriods.add(i);
              }
